Public Module EncryptionModule
    ...
    Public Function msEncryptTool2014(ByVal prmdataToEncrypt As String, ByVal prmEncCode As String) As String
        ...
        ' enccode must be 5 to 10 digits
        ' data to encrypt must be more than 4 digits
        ' 2014-10-20
    ...
    Public Function msDecryptTool2014(ByVal prmEncryptedData As String, ByVal prmEncCode As String) As String

        msDecryptTool2014 = ""   ' 0 means wrong code

        If prmEncryptedData.Length < 24 Or prmEncCode.Length < 5 Then   ' encrypted must be min 24 digits
            Exit Function
        End If

        If prmEncryptedData.Length Mod 2 = 1 Then
            Exit Function    ' len must be even
        ...
        modcode = Mid(prmEncryptedData, 1, 2)

        Dim modVal As Integer

        Dim i As Integer
        For i = 1 To 1024
            If ar(i) = modcode Then
                modVal = i
                Exit For
            End If
        Next
        ...
        If modVal < 13 Then
            Exit Function      ' mod val must be >= 13
        End If

        ' get random no
        Dim rncode As String
        Dim rnval As Integer

        rncode = Mid(prmEncryptedData, 3, 2)
        ...
        If rnval < 10 Or rnval > 99 Then
            Exit Function      ' mod val must be 10 to 99
        End If

        Dim forcs As Integer
        forcs = rnval
        ...
        For j = 5 To prmEncryptedData.Length Step 2
            dcode = Mid(prmEncryptedData, j, 2)
            dval = 0
            For i = 1 To 1000
                If ar(i) = dcode Then
                    dval = i
                    ...
                    k = k + 1
                    If k > 10 Then k = 1
                    If actval < 0 Or actval > 255 Then
                        Exit Function
                    End If
                    retVal = retVal & Chr(actval)
                    Exit For
                End If
            Next
            If dval = 0 Then Exit Function
        Next

        ' received ret val but do checksum
        If modVal = forcs Mod 678 + 13 Then

            If retVal.Length = 10 Then
                msDecryptTool2014 = retVal.TrimEnd
            Else
                msDecryptTool2014 = retVal
            End If
        End If
